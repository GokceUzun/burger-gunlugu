...
CATEGORIES = ["Ekmek", "Köfte", "Peynir", "Sos", "Yan Ürün", "Ortam/Servis", "Fiyat/Performans"]
...
def load_data() -> pd.DataFrame:
    if DATA_FILE.exists():
        ...
        df = pd.DataFrame(columns=[
            "id", "tarih", "mekan", "semt", "burger", "fiyat", "bekleme_dk",
...
with st.form("new_entry", clear_on_submit=True):
    ...
    for i, cat in enumerate(CATEGORIES):
        with cols_g[i % 3]:
            gokce_scores[cat] = st.slider(f"Gökçe – {cat}", 0, 10, 0, step=1)
    gokce_yorum = st.text_area("Gökçe Yorumu", placeholder="Kısa not…")
    ...
    for i, cat in enumerate(CATEGORIES):
        with cols_b[i % 3]:
            baba_scores[cat] = st.slider(f"Baba – {cat}", 0, 10, 0, step=1)
    baba_yorum = st.text_area("Baba Yorumu", placeholder="Kısa not…")
    baba_toplam = calc_total(baba_scores)

    genel_ortalama = round((gokce_toplam + baba_toplam)/2, 2)

    st.info(f"**Genel Ortalama: {genel_ortalama}**  |  Gökçe: {gokce_toplam}  •  Baba: {baba_toplam}")
...
# ---- Arama / Sıralama / Görünüm ----
st.header("📚 Kayıtlar")
col_f1, col_f2, col_f3 = st.columns([1.5,1,1])
with col_f1:
    q = st.text_input("Ara (mekan, semt, yorum)", "")
with col_f2:
    sort_key = st.selectbox("Sırala", options=[
        "tarih","ortalama","fiyat","mekan","semt","gokce_toplam","baba_toplam"
    ], index=1)
with col_f3:
    sort_dir = st.selectbox("Yön", options=["Azalan","Artan"], index=0)

view_df = df.copy()
if q.strip():
    mask = (
        view_df["mekan"].str.contains(q, case=False, na=False) |
        view_df["semt"].str.contains(q, case=False, na=False) |
        view_df["burger"].str.contains(q, case=False, na=False) |
        view_df["gokce_yorum"].str.contains(q, case=False, na=False) |
        view_df["baba_yorum"].str.contains(q, case=False, na=False)
    )
    view_df = view_df[mask]

# fiyata sayısal bak (sıralama için)
if sort_key == "fiyat":
    view_df["_fiyat_num"] = view_df["fiyat"].apply(safe_float)
    sk = "_fiyat_num"
else:
    sk = sort_key

ascending = (sort_dir == "Artan")
view_df = view_df.sort_values(by=sk, ascending=ascending)
